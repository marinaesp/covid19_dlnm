...
```{r}
master_dat <- covid_df |> left_join(pollution_df, by = c("mcp_code",
                                                   "week_num")) |> 
  left_join(vaccine_df, by = c("mcp_code",
                                                   "week_num")) |> 
  select(-X)
```
...
```{r}
correlations_ms <- round(cor(master_dat[,c(6, 8:13)], use = "pairwise.complete.obs"),1)
pvals_ms <- cor_pmat(master_dat[,c(6, 8:13)], digits = 2)

colnames(correlations_ms) <- c("Cases",  "O3", "BC", "NO2", "PM10", "PM2.5", "Vaccinations")
rownames(correlations_ms) <- c("Cases", "O3", "BC", "NO2", "PM10", "PM2.5", "Vaccinations")
