...
Since week 53 is just the 31st of December, I will change these records to week 52.
...
pollution <- ozone_prep |> 
  left_join(bc_prep[,c(1,2,5,6)], by = c("mcp_code", "month", "day")) |> 
  left_join(no2_prep[,c(1,2,5,6)], by = c("mcp_code", "month", "day")) |> 
  left_join(pm10_prep[,c(1,2,5,6)], by = c("mcp_code", "month", "day")) |> 
   left_join(pm25_prep[,c(1,2,5,6)], by = c("mcp_code", "month", "day")) |> 
...
pollution_week <- pollution |> 
  ...
  group_by(mcp_code, city_name, week_num) |> 
...
I noticed also that year 2021 had 52 full weeks, but the function `lubridate::week()` in R counts the 31st of December as week 53. I will remove this day or change it to also week 52, otherwise, we will be comparing 1 day measurements versus 7 day measurements.
...
```{r}
filter(master_dat, is.na(cumul_week))$mcp_code
```


# Check collinearity in the complete dataset

Check also for collinearity of pollution and vaccination data and with cases data

```{r}
correlations_ms <- round(cor(master_dat[,c(6, 7, 9:14)], use = "pairwise.complete.obs"),1)
pvals_ms <- cor_pmat(master_dat[,c(6,7, 9:14)], digits = 2)

colnames(correlations_ms) <- c("Cases", "Pop",  "O3", "BC", "NO2", "PM10", "PM2.5", "Vaccinations")
rownames(correlations_ms) <- c("Cases","Pop","O3", "BC", "NO2", "PM10", "PM2.5", "Vaccinations")
