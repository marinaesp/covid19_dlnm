...
for(i in 1:length(formulas))
  ...
  table0$logscore[i] <- round(-mean(log(model$cpo$cpo), na.rm=T), 4)
}
...
formula.multmodel4 <-update.formula(baseformula, ~ . + basis_bc + basis_ozone +  basis_vaccine)
formula.multmodel5 <-update.formula(baseformula, ~ . + basis_bc + basis_vaccine)
```

```{r}
formulas_mult <-list(formula.bc, formula.multmodel1, formula.multmodel2, formula.multmodel3, formula.multmodel4,formula.multmodel5 )
labels_mult <-c("model.bc", "model.bc_ozone", "model.bc_ozone_pm10", "model.bc_ozone_pm10_vaccine",
                "model.bc_ozone_vaccine", "model.bc_vaccine")
```
...
```{r}
table1 <- data.table::data.table(Model  = c("model_bc", "bc_ozone", "bc_ozone_pm10","bc_ozone_vaccine", "bc_ozone_vaccine", "bc_vaccine"),
                     DIC = NA,
                     logscore = NA)
...
An  additional model: base + bc +pm25 (it crushed during the previous runs)
